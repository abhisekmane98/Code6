...
int main()
{
	...
	// data stored from file for update
	...
	// updated data to add in file
	// file pointer is pointing at the end so to overwrite data we have to move it at start
	fseek(fp,0,SEEK_SET);	
	fwrite(&s3,sizeof(s3),1,fp);
	fclose(fp);	
	
	// to check final status of file
	fp=fopen("81_sorting_student.dat","rb");
